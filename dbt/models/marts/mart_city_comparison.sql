...
aggregated as (
    ...
    select
        ...
        count(*) filter (where has_area) as requests_with_area,
        round(
            100.0 * count(*) filter (where has_area) / nullif(count(*), 0),
            2
        ) as area_coverage_rate_pct,

        count(*) filter (where is_category_mapped) as mapped_category_requests,
        round(
            100.0 * count(*) filter (where is_category_mapped) / nullif(count(*), 0),
            2
        ) as category_mapping_rate_pct,
